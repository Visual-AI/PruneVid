...
api_key_list = [os.environ['OPENAI_API_KEY']]
...
true_list = [True] * len(api_key_list)
api_key_dict = dict(zip(api_key_list, true_list))
...
def check_ans(gt, pred, question, sub_task_type, question1=None, pred1=None, model="gpt-3.5-turbo-0125"):
    ...
    # print('-' * 10 + f'gt: {gt}')
    success, flag, score = False, False, 0
    for _ in range(3):
        try:
            ...
            # print(f"GPT took {time.time() - time1} seconds to respond")
    ...
    if task_type == 'generic_qa':
        ...
        for sub_task_type in ('context', 'detailed_orientation', 'correctness'):
            if pred=="":
                ...
                success, acc, score = check_ans(gt=gt, pred=pred, question=question, sub_task_type=sub_task_type, model=model) # acc is bool, score is given by chatgpt
...
def save_results(result_list, save_path, model="gpt-3.5-turbo-0125"):
    ...
    dump_json(result_list, save_path, 'eval_results.json')
